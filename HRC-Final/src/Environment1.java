...
public class Environment1 {
	...
	//i changed this to add robots to the arrayList, depending on the "R" we observed
	public Environment1(LinkedList<String> map, ArrayList<Robot> robotsGiven) { // modify for multiple robots MIW
		...
		for (int row = 0; row < this.rows; row++) {
			for (int col = 0; col < this.cols; col++) {
				...
				case 'R':
					...
					//robotsGiven.add(new Robot(this,row,col));
	...
	public int getNumDirtyTiles() {
		...
		for (int row = 0; row < this.rows; row++) {
			for (int col = 0; col < this.cols; col++) {
				if(this.getTileStatus(row,col).equals(TileStatus.DIRTY))
					//
					ret++;
			}
		}
